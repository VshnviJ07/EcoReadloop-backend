...
router.post("/signup", authLimiter, async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: "OTP sent to email/mobile", userId: user._id });
  } catch (err) {
...
router.post("/signin", authLimiter, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Signin successful", token, user: { id: user._id, fullName: user.fullName, email: user.email, mobile: user.mobile } });
  } catch (err) {
...
router.post("/verify-signin-otp", async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: "7d" });
    res.json({ success: true, message: "Signin verified successfully", token, user: { id: user._id, fullName: user.fullName, email: user.email, mobile: user.mobile } });
  } catch (err) {
...
router.patch("/profile", auth, async (req, res) => {
  try {
    const allowedFields = ["fullName", "email", "mobile", "alternateMobile", "age", "dob", "address", "city", "state", "pincode", "gender"];
    const updates = {};
