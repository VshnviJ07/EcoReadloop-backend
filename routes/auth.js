...
// Utility: send OTP (here console.log for now)
...
router.post("/signup", authLimiter, async (req, res) => {
  try {
    ...
    if (!fullName || !identifier || !password) return res.status(400).json({ success: false, message: "Required fields missing" });
...
router.post("/verify-signup-otp", async (req, res) => {
  try {
    ...
    if (user.otp !== otp || user.otpExpires < new Date()) return res.status(400).json({ success: false, message: "Invalid or expired OTP" });
...
router.post("/signin", authLimiter, async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: "7d" });
    res.json({ success: true, token, user: { id: user._id, fullName: user.fullName, email: user.email, mobile: user.mobile } });
  } catch (err) {
    res.status(500).json({ success: false, message: "Server error" });
  }
});
...
router.patch("/profile", auth, async (req, res) => {
  try {
    const allowed = ["fullName","email","mobile","alternateMobile","age","dob","address","city","state","pincode","gender"];
    const updates = {};
...
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    console.log(`Forgot Password OTP for ${identifier}: ${otp}`); // send email/SMS in real app
